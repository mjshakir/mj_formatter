...
class PolicyFactory:
    ...
    def describe(self, name: str, settings: dict[str, object]) -> dict[str, str]:
        ...
        if policy_type == "python":
            if name in self._registry.names():
                ...
                if cls:
                    return {
                        "type": "python",
                        "parse_mode": getattr(cls, "parse_mode", "text"),
                        "description": str(getattr(cls, "description", "")),
                    }
            return {"type": "python", "parse_mode": "text", "description": ""}
        if policy_type == "lua":
            return {"type": "lua", "parse_mode": "text", "description": "Lua policy"}
        if policy_type in self._type_map and self._type_map[policy_type] is not None:
            cls = self._type_map[policy_type]
            return {
                "type": policy_type,
        ...
        return {"type": policy_type, "parse_mode": "text", "description": ""}
