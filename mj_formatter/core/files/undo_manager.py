...
from ..types import AppConfig, CollectTargetsArgs, FileIOConfig


class UndoManager:
    CollectTargetsArgs = CollectTargetsArgs

    def __init__(self, config: FileIOConfig, toml_loader: TomlStore | None = None) -> None:
    ...
    def _load_manifest(self, run_dir: Path) -> dict[str, str] | None:
        ...
        try:
            data = self._toml_loader.load(manifest_path)
        except Exception:
            return None
    ...
    def latest_manifest_targets(self) -> list[Path]:
        run_dirs = self._list_run_dirs()
        for run_dir in run_dirs:
            manifest = self._load_manifest(run_dir)
            if not manifest:
            ...
            for rel in manifest.keys():
                try:
                    targets.append((self._root / rel).resolve())
                except Exception:
                    continue
            if targets:
                return targets
    ...
    def restore_all(
        ...
        targets: list[Path],
        delete_backup: bool,
        ignore_missing_backups: bool = False,
    ) -> tuple[int, list[str]]:
        restored = 0
        errors: list[str] = []
        for target in targets:
            ...
            if ok:
                ...
                if ignore_missing_backups and err and err.startswith("no backup found"):
                    continue
