    ...
    class BitmaskTable {
        ...
        protected:
            ...
            std::enable_if_t<(M == 0) or (M > 64), std::optional<IndexType>> acquire_data(void) {
                ...
                thread_local size_t part_hint = 0ULL;
                thread_local uint8_t bit_hint = 0U;
            ...
            scan_available(const size_t& start_part, const size_t& mask_count_size, const size_t& available_plane) {
                ...
                const bool _use_tree = tree_enabled();
                BitmapTree* tree     = _use_tree ? tree_ptr() : nullptr;
                for (size_t offset = 0ULL; offset < mask_count_size; ++offset) {
                    //--------------------------
            ...
            invalid_bits(const IndexType& capacity, const IndexType& mask_count) {
                ...
                if (valid_bits < C_BITS_PER_MASK) {
                    //--------------------------
                    const uint64_t valid_mask   = (valid_bits == 0ULL) ? 0ULL : ((1ULL << valid_bits) - 1ULL);
